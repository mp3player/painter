import { Stack } from "../container/list/collection.js";
...
class GMath {
    ...
    static Geometry = {
         ...
         * @returns 
         */
        FindEar : ( edge : Array< Vector3 > , anti : boolean  ) : Array< number > => {
         ...
         * @returns 
         */
        Triangulate : ( path : Array< Vector3 > , anti = true ) : Array< Array< Vector3 > > => {
            ...
            while( edge.length >= 3 ){
                // console.log( edge )
                let ear = GMath.Geometry.FindEar( edge , anti );
         ...
         * @param poly1 
         */
        GJKDetection : ( poly0 : Array< Vector3 > , poly1 : Array< Vector3 > ) => {
         ...
         */
        ConvertToBaryCentricCoordinate : ( A : Vector3 , B : Vector3 , C : Vector3 , P : Vector3 ) : Vector3 => {
        ...
        // geometry 
        getSlop : ( v0 : Vector3 , v1 : Vector3) : number => {
            if( v0.x == v1.x ) return Number.POSITIVE_INFINITY;
            return ( v1.y - v0.y ) / ( v1.x - v0.x );
        },
    
        // intersection( line0 , line1 )
        isLineIntersected : ( p0 : Vector3, p1 : Vector3, p2 : Vector3, p3 : Vector3 ) : boolean => {
        ...
        IsPointInCircle : ( center : Vector3, radius : number  , point : Vector3) : boolean => {
        ...
        IsPoingInEllipse : ( a : number , b : number , point : Vector3) : boolean => {
            return false;
        },
    
    
        isPointInLine : ( p0 : Vector3, p1 : Vector3, point : Vector3, eps = 1.0 ) : boolean => {
        ...
        isPointInPath : ( edge : Array< Vector3> , point : Vector3, eps = 1.0) : boolean => {
            for( let i = 1 ; i < edge.length ; ++i ){
                if( this.isPointInLine( edge[ i - 1 ] , edge[i] , point , eps ) ) return true;
            }
            return false;
        ...
        IsPointInRect : ( edge : Array<Vector3> , point : Vector3) : boolean => {
        ...
        IsPointInPolygon : ( edge : Array<Vector3> , point : Vector3 ) : boolean => {
